...
const CarouselElement = ({ title, movies }) =>  {
    ...
    const handleNext = () => {
        ...
        if(window.innerWidth > 768) {
            if(Math.abs(carouselValue) <= carouselMaxWidth - (window.innerWidth / 1.5)) setCarouselValue(carouselValue - 250);
        }
        else {
    ...
    return (
        <>
            <div data-id={title.replaceAll(" ", "").toLowerCase()} className="movieCarousel relative w-full mb-10">
                <h2 className="font-main text-2xl text-gray-100 font-semibold drop-shadow-lg mb-4">{title}</h2>
                <div id="carouselControls" className="flex gap-6 w-[100px] absolute top-0 right-0">
                    <button type="button" id="prevButton" className="flex justify-center items-center text-white bg-mainRed rounded-full h-10 w-10 border-0 drop-shadow-md transition-all hover:bg-darkGray" disabled={carouselValue === 0 ? true : false} onClick={handlePrev}>
                        <AiOutlineArrowLeft size="1.4em" />
                    </button>
                    <button id="nextButton" className="flex justify-center items-center text-white bg-mainRed rounded-full h-10 w-10 border-0 drop-shadow-md transition-all hover:bg-darkGray" onClick={handleNext}>
                        <AiOutlineArrowRight size="1.4em" />
                    </button>
                </div>
                <ul ref={carouselRef} id="carousel" className="flex justify-start items-center w-full gap-4 transition-all">
                    {
